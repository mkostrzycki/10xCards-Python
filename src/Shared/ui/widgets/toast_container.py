...
class ToastContainer(ttk.Frame):
    ...
    def show_toast(self, title: str, message: str) -> None:
        ...
        """
        toast = ToastNotification(id=self._next_id, title=title, message=message, created_at=time.time())
        self._next_id += 1
    ...
    def _show_toast_widget(self, toast: ToastNotification) -> None:
        ...
        """
        # Create toast frame
        toast_frame = ttk.Frame(self, style="primary.TFrame", padding=10)

        # Title
        title_label = ttk.Label(
            toast_frame, text=toast.title, style="primary.Inverse.TLabel", font=("TkDefaultFont", 10, "bold")
        )
        title_label.pack(anchor=tk.W)

        # Message
        message_label = ttk.Label(
            toast_frame, text=toast.message, style="primary.Inverse.TLabel", wraplength=self.TOAST_WIDTH - 40
        )
        ...
        toast_frame.place(x=0, y=y_position, width=self.TOAST_WIDTH, height=self.TOAST_HEIGHT)
    ...
    def _reposition_toasts(self) -> None:
        ...
        active_toasts = sorted(self._active_toasts.values(), key=lambda t: t.created_at)

        for i, toast in enumerate(active_toasts):
            if toast.widget:
                y_position = i * (self.TOAST_HEIGHT + self.TOAST_MARGIN)
                toast.widget.place(x=0, y=y_position, width=self.TOAST_WIDTH, height=self.TOAST_HEIGHT)
