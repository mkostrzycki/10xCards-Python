...
def service(mock_flashcard_repository, mock_review_log_repository, mock_session_service):
    with patch('src.Study.application.services.study_service.get_config') as mock_get_config:
        ...
        mock_get_config.return_value = {
            "FSRS_DEFAULT_PARAMETERS": [0.4, 0.6, 2.4, 5.8, 4.93, 0.94, 0.86, 0.01, 1.49, 0.14, 0.94, 2.18, 0.05, 0.34, 1.26, 0.29, 2.61],
        ...
        service = StudyService(
            mock_flashcard_repository,
            mock_review_log_repository,
...
def test_record_review_updates_flashcard_and_saves_log(service, mock_flashcard_repository, mock_review_log_repository, sample_flashcards):
    # Arrange
    flashcard_id = 1
    rating = 3  # Good
...
def test_record_review_validates_rating(service):
    ...
    # Symulujemy aktywną sesję nauki
    mock_fsrs_card = MagicMock()
    service.current_study_session_queue = [(Flashcard(id=1, deck_id=1, front_text="Test", back_text="Test", source="manual"), mock_fsrs_card)]
    service.current_card_index = 0
